...
class BookDetailsCard {
  static void show(BuildContext context, Book book, String selectedListName, BookService bookService, String? selectedListId) {
                ...
                children: [
                  ...
                  if (book.description.length > 100)
                    TextButton(
                      ...
                      child: Text(isDescriptionExpanded ? 'Collapse' : 'Expand'),
                    ),
                  const SizedBox(height: 16),
                  Center(
                    child: Column(
                          ...
                          children: List.generate(5, (index) {
                            return IconButton(
                              ...
                              onPressed: () async {
                                ...
                                print('Setting rating to $newRating stars for ${book.title}');
                                setModalState(() {
                                  currentRating = newRating;
                                });
                                try {
                                  await bookService.updateBookRating(book.isbn, book.title, newRating, selectedListName, selectedListId);
                                } catch (e) {
                                  setModalState(() {
                                    currentRating = book.userRating; // Revert on error
                                  });
                                }
                              },
                            );
                    ...
                    children: [
                      TextButton(
                        onPressed: isDeleting
                            ? null
                            : () async {
                                print('Delete button pressed for: ${book.title}');
                                setModalState(() {
                                  isDeleting = true;
                                  print('Set isDeleting = true');
                                });
                                try {
                                  await bookService.deleteBook(book.isbn, book.title, selectedListName, selectedListId);
                                  print('Delete operation completed');
                                  if (modalContext.mounted) {
                                    ScaffoldMessenger.of(modalContext).showSnackBar(
                                      SnackBar(
                                        content: Text('Book deleted from $selectedListName: ${book.title}'),
                                      ),
                                    );
                                  }
                                } catch (e, stackTrace) {
                                  ...
                                  print('Entering finally block for book delete');
                                  if (modalContext.mounted) {
                                    setModalState(() {
                                      isDeleting = false;
